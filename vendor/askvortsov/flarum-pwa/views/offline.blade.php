...
<head>
  ...
  <script>
    function show_lalao() {
      var contentDiv = document.getElementById('where_is_lalao');
      if (contentDiv.classList.contains('where_goes_lalao')) {
        contentDiv.classList.remove('where_goes_lalao');
        contentDiv.classList.add('lalao_display');
      } else {
        contentDiv.classList.remove('lalao_display');
        contentDiv.classList.add('where_goes_lalao');
      }
    }
  </script>
</head>
...
<body>
  <div class="container">
    <div class="box">
      ...
      <h1>破站💊！</h1>
      <p>服务器已堕入虚空…… 开发组正在化身为毁灭战士！</p>
      <p>什么？<span class="laji">辣佬？</span> <a href="#" onclick="show_lalao()">#查询辣佬正在做什么</a></p>
      <div id="where_is_lalao" class="where_goes_lalao">
        </br>
        <p>👇索引返回了以下结果</p>
        <img class="lalao_escaping" src="https://ptpimg.me/25nn16.png">
      </div>
      </br>
      <p>Invites.fun Lives Matter!</p>
      <p>
        点此加入<a
          href="http://qm.qq.com/cgi-bin/qm/qr?_wv=1027&k=05aL6XEZbVP9EH3Q33pwxiqdUYcuYddw&authKey=Trdh%2BbY%2FdrVat5%2FmBGMcnbYw6Gy3vevxok6cziOvMkPF4ihv%2BXelGcnUMmD9eq53&noverify=0&group_code=779320815"
          target="_blank">QQ群</a>
        药丸后援组
      </p>
      </br>
...
<script>
    ...
    if (typeof is_takeover == 'undefined') {
        setInterval(() => {
            if (navigator.onLine) {
                location.host == 'invites.fun' ? location.href = 'https://www.invites.fun' + location.pathname : location.reload();
            }
        }, 1000 * 60);
    }
  </script>
